...
`simd-multi/` layers **OpenMP** on top of the same AVX2 kernels:
- Each thread owns isolated scratch buffers (no false sharing)
- `#pragma omp parallel for` distributes vectors across cores
- Each core executes AVX2 kernels on its assigned slice

This gives two levels of parallelism:
1. **Across cores** — OpenMP thread team
2. **Within a core** — AVX2 vector lanes
